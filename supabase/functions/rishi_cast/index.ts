...
async function generatePDF(data: ExportData): Promise<Uint8Array> {
  ...
  // Define colors
  const goldColor = rgb(0.89, 0.77, 0.29);  // #e4c44a
  const darkBrown = rgb(0.36, 0.27, 0.18);   // #5c452e
  const textColor = rgb(0.2, 0.15, 0.1);     // Dark text
  const lightGray = rgb(0.4, 0.4, 0.4);      // For secondary text
  ...
  // Title
  y = addText(t.exportTitle, 24, helveticaBold, darkBrown, margin, y);
  y -= 10;
